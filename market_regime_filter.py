...
# Step 3: Define the mean reversion trading strategy with marker regime filter
class RollingIBSStrategy(Strategy):
    ...
    def next(self):
        if self.data.Close[-1] > self.sma_300[-1]:  # Bull market condition
            ...
            # Bear market condition: Close any open positions and avoid new trades
            if self.position:
                self.position.close()
...
plt.title("Performance: Marker Regime Filter Strategy vs. Buy & Hold (Log Scale)")
plt.xlabel("Time")
...
plt.title("Drawdown: Marker Regime Filter Strategy vs. Buy & Hold")
plt.xlabel("Time")
plt.ylabel("Drawdown (%)")
plt.legend()
