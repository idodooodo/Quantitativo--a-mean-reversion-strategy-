...
# Step 3: Define the mean reversion trading strategy
class RollingIBSStrategy(Strategy):
    ...
    def init(self):
        ...
        # Compute the lower band of rolling high over 10 days minus 2.5 times rolling mean
    ...
    def next(self):
        ...
        # Close condition
        if self.data.Close > self.data.High[-2] :
            self.position.close()
...
# Extract the equity curve (cumulative value of the portfolio over time).
strategy_equity = stats['_equity_curve']['Equity']

initial_cash = 100_000
buy_and_hold = data['Close'] / data['Close'].iloc[0] * initial_cash  # Normalize to start at 100
...
plt.title("Performance: First Strategy vs. Buy & Hold (Log Scale)")
plt.xlabel("Time")
...
strategy_peak = strategy_equity.cummax()  # Peak equity up to each time point
...
buy_and_hold = buy_and_hold.reindex(strategy_equity.index, method='nearest')  # Align indices

buy_and_hold_peak = buy_and_hold.cummax()  # Peak equity up to each time point
...
plt.title("Drawdown: First Strategy vs. Buy & Hold")
plt.xlabel("Time")
plt.ylabel("Drawdown (%)")
plt.legend()
